	...
	<view class="all">
		...
		<form @submit="formSubmit">
			...
			<textarea class="content" name="textarea" :maxlength="-1" type="text" placeholder="今天发生了什么？心事详情在这里请填写吧~"
				auto-blur='true' :value="textarea"></textarea>
			<view class="images">
				<uni-file-picker v-model="imageValue" fileMediatype="image" mode="grid" 
					@success="success" @fail="fail" limit="1" :auto-upload="false" ref="files" />
			</view>
			<view class="day">
				<view>心事日期</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			fail() {
				uni.showModal({
					content: "上传图片失败，请检查网络"
				})
			},
			async formSubmit(e) {
				this.value = e.detail.value
				this.showProgress=true
				console.log(this.imageValue,"imageValue")
